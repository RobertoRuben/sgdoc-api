from typing import Dict, Any, List
from fastapi import Depends
from src.exception import NotFoundException, InternalServerException
from src.model.entity import EstadoDocumento
from src.repository import EstadoDocumentoRepository, DocumentoRepository
from src.dto import EstadoDocumentoRequestDTO, EstadoDocumentoResponseDTO
from src.service import EstadoDocumentoService


class EstadoDocumentoServiceImp(EstadoDocumentoService):
    def __init__(
        self,
        estado_documento_repository: EstadoDocumentoRepository = Depends(),
        documento_repository: DocumentoRepository = Depends()
    ):
        self.estado_documento_repository = estado_documento_repository
        self.documento_repository = documento_repository


    async def add(self, estado_documento_request: EstadoDocumentoRequestDTO) -> EstadoDocumentoResponseDTO:
        try:
            exists_doc = await self.documento_repository.exists_by_id(estado_documento_request.documento_id)
            if not exists_doc:
                raise NotFoundException("Documento no encontrado")

            estado_documento = EstadoDocumento(
                estado=estado_documento_request.estado,
                comentario=estado_documento_request.comentario,
                documento_id=estado_documento_request.documento_id
            )

            created_estado_documento = await self.estado_documento_repository.add_estado_documento(estado_documento)

            return EstadoDocumentoResponseDTO(
                id=created_estado_documento.id,
                estado=created_estado_documento.estado,
                comentario=created_estado_documento.comentario,
                fecha=created_estado_documento.fecha
            )
        except NotFoundException as e:
            raise e
        except Exception as e:
            raise InternalServerException(
                detail="Error interno al crear el estado del documento",
                error_details=str(e)
            ) from e


    async def get_all(self, page: int = 1, page_size: int = 10) -> Dict[str, Any]:
        try:
            return await self.estado_documento_repository.get_all_estado_documento(page, page_size)
        except Exception as e:
            raise InternalServerException(
                detail="Error interno al obtener los estados de los documentos",
                error_details=str(e)
            ) from e


    async def update(self, estado_documento_id: int, estado_documento_request: EstadoDocumentoRequestDTO) -> EstadoDocumentoResponseDTO:
        try:
            estado_documento = await self.estado_documento_repository.get_estado_documento_by_id(estado_documento_id)
            if not estado_documento:
                raise NotFoundException("Estado del documento no encontrado")

            exists_doc = await self.documento_repository.exists_by_id(estado_documento_request.documento_id)
            if not exists_doc:
                raise NotFoundException("Documento no encontrado")

            estado_documento.estado = estado_documento_request.estado
            estado_documento.comentario = estado_documento_request.comentario
            estado_documento.documento_id = estado_documento_request.documento_id

            updated_estado_documento = await self.estado_documento_repository.update_estado_documento(estado_documento)

            return EstadoDocumentoResponseDTO(
                id=updated_estado_documento.id,
                estado=updated_estado_documento.estado,
                comentario=updated_estado_documento.comentario,
                fecha=updated_estado_documento.fecha
            )
        except NotFoundException as e:
            raise e
        except Exception as e:
            raise InternalServerException(
                detail="Error interno al actualizar el estado del documento",
                error_details=str(e)
            ) from e


    async def delete_by_id(self, estado_documento_id: int) -> None:
        try:
            exists = await self.estado_documento_repository.exists_estado_documento_by_id(estado_documento_id)
            if not exists:
                raise NotFoundException("Estado del documento no encontrado")
            await self.estado_documento_repository.delete_by_id(estado_documento_id)
        except NotFoundException as e:
            raise e
        except Exception as e:
            raise InternalServerException(
                detail="Error interno al eliminar el estado del documento",
                error_details=str(e)
            ) from e


    async def get_all_by_documento_id(self, documento_id: int) -> List[EstadoDocumento]:
        try:
            return await self.estado_documento_repository.get_all_estados_by_id(documento_id)
        except Exception as e:
            raise InternalServerException(
                detail="Error interno al obtener los estados del documento",
                error_details=str(e)
            ) from e
